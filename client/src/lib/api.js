...
export const places = {
  ...
  newSuggestions:()=>req("/places/suggestions/new"),
  suggestNew:(changes, reason="")=>req("/places/suggestions/new",{method:"POST",body:JSON.stringify({ changes, reason })}),
  acceptNewSuggestion:id=>req(`/places/suggestions/new/${id}/accept`,{method:"POST"}),
  rejectNewSuggestion:id=>req(`/places/suggestions/new/${id}/reject`,{method:"POST"}),
  citationExclusions:slug=>req(`/places/${encodeURIComponent(slug)}/citation-exclusions`),
  excludeCitation:(slug, workSlug, lineNumber, lineText="")=>req(`/places/${encodeURIComponent(slug)}/citation-exclusions`,{method:"POST",body:JSON.stringify({ workSlug, lineNumber, lineText })}),
